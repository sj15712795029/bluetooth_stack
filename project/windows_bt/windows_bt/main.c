...
uint8_t shell_parse(uint8_t *shell_string)
{
    ...
    if(hw_strncmp(BT_START_CMD,(const char*)shell_string,hw_strlen(BT_START_CMD)) == 0)
    {
    ...
    if(hw_strncmp(BT_STOP_CMD,(const char*)shell_string,hw_strlen(BT_STOP_CMD)) == 0)
    {
...
DWORD WINAPI stdin_process_thread(LPVOID p)
{	
	while(1)
	{
		memset(stdin_buf,0,STDIN_BUF_SIZE);
		fgets(stdin_buf,STDIN_BUF_SIZE,stdin);
		stdin_recv_len = strlen(stdin_buf);
		printf("stdin_buf:%s\n",stdin_buf);
	}
}
...
DWORD WINAPI timer_thread(LPVOID p)
{
    printf("timer_thread runing...\n");
    while(1)
    {
    	Sleep(1000/CONF_BSP_TICKS_PER_SEC);
...
{
	DWORD  thread_stdin_id;
	CreateThread(NULL, 0,stdin_process_thread, 0, 0, &thread_stdin_id);

}

void board_init()
{
    DWORD thread_timer_id;

    last_sys_time = sys_time;
    utimer_init();

	CreateThread(NULL, 0,timer_thread, 0, 0, &thread_timer_id);
