...
/* The L2CAP PCB lists. */
struct l2cap_pcb_listen_t *l2cap_listen_pcbs;  /* List of all L2CAP PCBs in CLOSED state
						but awaiting an incoming conn req */
struct l2cap_pcb_t *l2cap_active_pcbs;  /* List of all L2CAP PCBs that are in a
					 state in which they accept or send
					 data */
struct l2cap_pcb_t *l2cap_tmp_pcb;

/* Temp signal */
struct l2cap_sig_t *l2cap_tmp_sig;
...
static uint8_t l2cap_next_sigid(void);
static err_t l2cap_signal(struct l2cap_pcb_t *pcb, uint8_t code, uint16_t ursp_id, struct bd_addr_t *remote_bdaddr, struct bt_pbuf_t *data);
static err_t l2cap_rexmit_signal(struct l2cap_pcb_t *pcb, struct l2cap_sig_t *sig);
...
void l2cap_tmr(void)
{
    struct l2cap_sig_t *sig;
    struct l2cap_pcb_t *pcb;
    err_t ret = BT_ERR_OK;
    BT_UNUSED_ARG(ret);
                ...
                if(sig->rtx > 0)
                {
                    ...
                    if(sig->rtx == 0)
                    {
                        if(sig->nrtx == 0)
                        {
                            ...
                            L2CA_ACTION_DISCONN_IND(pcb,BT_ERR_OK,ret);
                        }
                        else
                        {
                            --sig->nrtx;
                            /* Indicate timeout to upper layer */
                            L2CA_ACTION_TO_IND(pcb,BT_ERR_OK,ret);
        ...
        if(pcb->state == L2CAP_CONFIG)
        {
            ...
            if(pcb->cfg.cfgto > 0)
            {
                ...
                if(pcb->cfg.cfgto == 0)
                {
                    ...
                    {
                        L2CA_ACTION_CONN_IND(pcb, BT_ERR_OK, ret);
                    }
...
err_t l2cap_write(struct bd_addr_t *bdaddr, struct bt_pbuf_t *p, uint16_t len)
{
    ...
    for(q = p; q != NULL; q = q->next) {
      for(i = 0; i < q->len; ++i) {
      BT_L2CAP_TRACE_DEBUG("l2cap_write: 0x%x\n", ((uint8_t *)q->payload)[i]);
      }
      BT_L2CAP_TRACE_DEBUG("l2cap_write: *\n");
      }
    */
...
void l2cap_process_sig(struct bt_pbuf_t *q, struct l2cap_hdr_t *l2caphdr, struct bd_addr_t *bdaddr)
{
    ...
    struct l2cap_sig_t *sig = NULL;
    struct l2cap_pcb_t *pcb = NULL;
    struct l2cap_pcb_listen_t *lpcb;
    struct l2cap_cfgopt_hdr_t *opthdr;
    uint16_t result, status, flags, psm, dcid,info_type;
    uint16_t scid = 0;
    uint16_t len;
    uint16_t siglen;
    struct bt_pbuf_t *p, *r = NULL, *s = NULL, *data;
    err_t ret = BT_ERR_OK;
    uint8_t i;
    ...
    if(q->len != q->tot_len)
    {
        BT_L2CAP_TRACE_DEBUG("l2cap_process_sig: Fragmented packet received. Reassemble into one buffer\n");
        if((p = bt_pbuf_alloc(BT_PBUF_RAW, q->tot_len, BT_PBUF_RAM)) != NULL)
        {
            i = 0;
    ...
    while(len > 0)
    {
        ...
        case L2CAP_INFO_REQ:
        {
            info_type = ((uint16_t *)p->payload)[0];
            ...
            case L2CAP_CONLESS_MTU:
                ...
                /* Send the response */
                ret = l2cap_signal(pcb, L2CAP_INFO_RSP, sighdr->id, bdaddr, data);
                break;
        ...
        break;
        case L2CAP_INFO_RSP:
            BT_L2CAP_TRACE_DEBUG("l2cap_process_sig: infomation response\n");
...
void l2cap_acl_input(struct bt_pbuf_t *p, struct bd_addr_t *bdaddr)
{
    ...
    case L2CAP_NULL_CID:
        ...
        break;
    case L2CAP_SIG_CID:
        bt_pbuf_header(inseg->p, -L2CAP_HDR_LEN);
    ...
    default:
        if(inseg->l2caphdr->cid < 0x0040 || inseg->pcb == NULL)
        {
            ...
            {
                ((uint16_t *)data->payload)[0] = L2CAP_INVALID_CID;
                ((uint16_t *)data->payload)[1] = inseg->l2caphdr->cid;
        ...
        L2CA_ACTION_RECV(inseg->pcb,inseg->p,BT_ERR_OK,ret);
        break;
    }
...
struct l2cap_pcb_t *l2cap_new(void)
{
    ...
    if(pcb != NULL)
    {
        memset(pcb, 0, sizeof(struct l2cap_pcb_t));
        pcb->state = L2CAP_CLOSED;
...
{
    struct l2cap_sig_t *tmpsig;

    if(pcb->state == L2CAP_LISTEN)
    {
        L2CAP_RMV((struct l2cap_pcb_t **)&l2cap_listen_pcbs, pcb);
...
{
    struct l2cap_pcb_t *pcb, *tpcb;
    struct l2cap_pcb_listen_t *lpcb, *tlpcb;
    struct l2cap_seg_t *seg, *tseg;
    ...
    {
        tlpcb = lpcb->next;
        l2cap_close((struct l2cap_pcb_t *)lpcb);
        lpcb = tlpcb;
    }
...
static  err_t l2cap_signal(struct l2cap_pcb_t *pcb, uint8_t code, uint16_t ursp_id, struct bd_addr_t *remote_bdaddr,
                           struct bt_pbuf_t *data)
{
    struct l2cap_sig_t *sig;
    struct l2cap_sig_hdr_t *sighdr;
    struct l2cap_hdr_t *hdr;
    err_t ret;

    /* Alloc a new signal */
    BT_L2CAP_TRACE_DEBUG("l2cap_signal: Allocate memory for l2cap_sig_t. Code = 0x%x\n", code);
    ...
    sighdr = (struct l2cap_sig_hdr_t *)(((uint8_t *)sig->p->payload)+L2CAP_HDR_LEN);
    ...
    if(sighdr->code % 2)   /* If odd this is a resp/rej signal */
    {
        ...
        BT_L2CAP_TRACE_DEBUG("l2cap_signal: Sending response/reject signal with id = %d code = %d\n",
    ...
    else
    {
        ...
        BT_L2CAP_TRACE_DEBUG("l2cap_signal: Sending request signal with id = %d code = %d\n",
                             sig->sigid, sighdr->code);
    }
    ...
    {
        BT_L2CAP_TRACE_DEBUG("l2cap_signal: Registering sent request signal with id = %d code = %d\n",
                             sig->sigid, sighdr->code);
        L2CAP_SIG_REG(&(pcb->unrsp_sigs), sig);
    }
    else
    {
        BT_L2CAP_TRACE_DEBUG("l2cap_signal: Deallocating sent response/reject signal with id = %d code = %d\n",
                             sig->sigid, sighdr->code);
        bt_pbuf_free(sig->p);
        sig->p = NULL;
        bt_memp_free(MEMP_L2CAP_SIG, sig);
...
err_t l2cap_connect_req(struct l2cap_pcb_t *pcb, struct bd_addr_t *bdaddr, uint16_t psm,
                        uint8_t role_switch, err_t (* l2ca_connect_cfm)(void *arg, struct l2cap_pcb_t *lpcb,
...
err_t l2cap_config_req(struct l2cap_pcb_t *pcb)
{
...
err_t l2cap_disconnect_req(struct l2cap_pcb_t *pcb, err_t (* l2ca_disconnect_cfm)(void *arg, struct l2cap_pcb_t *pcb))
{
    struct bt_pbuf_t *data;
    err_t ret;
    ...
    else
    {
        if(err == BT_ERR_OK)
        {
            ...
            if((data = bt_pbuf_alloc(BT_PBUF_RAW, L2CAP_CONN_REQ_SIZE, BT_PBUF_RAM)) != NULL)
            {
                ...
                ((uint16_t *)data->payload)[1] = pcb->scid;
                if((ret = l2cap_signal(pcb, L2CAP_CONN_REQ, 0, &(pcb->remote_bdaddr), data)) == BT_ERR_OK)
                {
                    pcb->state = W4_L2CAP_CONNECT_RSP;
                }
                else
...
err_t l2cap_register_connect_ind(struct l2cap_pcb_t *npcb, uint8_t psm,
                                 err_t (* l2ca_connect_ind)(void *arg, struct l2cap_pcb_t *pcb, err_t err))
{
    ...
    bt_memp_free(MEMP_L2CAP_PCB, npcb);
    L2CAP_REG(&l2cap_listen_pcbs, lpcb);
    return BT_ERR_OK;
}

void l2cap_register_disconnect_ind(struct l2cap_pcb_t *pcb,
                                   err_t (* l2ca_disconnect_ind)(void *arg, struct l2cap_pcb_t *newpcb, err_t err))
{
