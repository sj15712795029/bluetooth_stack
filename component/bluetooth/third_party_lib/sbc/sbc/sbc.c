		...
		struct sbc_frame *frame, size_t len)
{
	...
	frame->mode = (sbc_mode_e)((data[1] >> 2) & 0x03);
	...
	frame->allocation = (sbc_allocation_e)((data[1] >> 1) & 0x01);
			...
			void *output, size_t output_len, ssize_t *written)
{
	...
	if (!priv->init) {
		...
		priv->frame.mode = (sbc_mode_e)(sbc->mode);
		priv->frame.channels = sbc->mode == SBC_MODE_MONO ? 1 : 2;
		priv->frame.allocation = (sbc_allocation_e)(sbc->allocation);
